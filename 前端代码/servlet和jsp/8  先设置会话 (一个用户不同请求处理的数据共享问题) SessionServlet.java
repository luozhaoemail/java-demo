 ...
 * 			共享的数据。
 * 		特点：
 * 			session存储在服务器端， 而Cookie存在浏览器端
 * 			服务器进行创建
 * 			依赖Cookie技术
 * 			有效期是一次会话
 * 			默认存储时间是30分钟，在tomcat/web.xml的<session-config>中配置
 *		作用：
 ...
 * 				如果session对象是失效了，也会重新创建一个session对象，并将其JSESSIONID作为cookie数据存储在浏览器内存中。
 * 			设置session存储时间
 * 				hs.setMaxInactiveInterval(int seconds);
 * 				注意：
 * 					在指定的时间内session对象没有被使用则销毁，如果使用了则重新计时。如果超过30分钟没用，要重新登陆。
 * 			设置session强制失效
 * 				hs.invalidate();
 * 			存储和获取数据
 ...
 * 			作用域：
 * 				一次会话
 * 				在JSESSIONID和SESSION对象不失效的情况下为整个项目内有效。
 * 			session失效处理：

 ...
 * 			步骤二：
 * 				在tomcat的目录下的conf目录中修改server.xml文件：在Connector=8080标签中增加属性 useBodyEncodingForURI="true"
 * 		post方式：
...
public class LoginServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		//处理请求信息
			//浏览器请求 --> LoginServlet --> 在这里获取业务层对象--> Dao层 --> POJO（数据库）
		...
		//响应处理结果
			if(u!=null){
				//创建Cookie信息实现三天免登录。
					...
					//添加Cookie信息
					resp.addCookie(c);
				//请求转发，一次请求 多次刷新地址栏不变，表单会重复提交,即多次调用本servlet
					//req.getRequestDispatcher("main").forward(req, resp);
				//重定向，两次请求，地址栏变了，刷新不会调本servlet
				resp.sendRedirect("/login/main");
				return;
			}else{
			...
			而getRequestDispatcher又分成两种:
				request.getRequestDispatcher("third?name=jay").forward(request, response);  //url可以是相对路径也可以是绝对路径。 
				this.getServletContext().getRequestDispatcher("/third?name=jay").forward(request, response);  //url只能是绝对路径。	

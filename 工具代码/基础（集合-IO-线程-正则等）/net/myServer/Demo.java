...
import javax.swing.*;
/**
 * 本小程序使用了几个简单常用的JLebel,JButton,JCombBox,JTextField,JSlider,JSpinner等等Swing控件，
 * 并且为JButton控件添加了监听器类，在点击JButton的时候在控制台输出HelloWorld。
 * 监听器类的行为由控件的事件触发。
 * 
 * 常用的布局控制器有FlowLayout,GridLayout,GridBagLayout,BorderLayout等等，
 * 使用方法setLayout(new FlowLayout()）来更改面板的布局控制器。
...
//        this.getContentPane().setLayout(new GridBagLayout());//设置布局控制器
        ...
        this.add(this.getSlider(),null); //滑动条
        this.add(this.getSpinner(),null);//数字调节控件
        this.add(this.getToolBar(),null);  //窗体上的控制条
    ...
    private JMenuBar getMenu(){
        if(menuBar==null){
            menuBar = new JMenuBar();//1
            JMenu m1 = new JMenu();//2
            m1.setText("文件");
            JMenu m2 = new JMenu();
            m2.setText("编辑");
            JMenu m3 = new JMenu();
            ...
            JMenuItem item11 = new JMenuItem();//3
            item11.setText("打开");
    ...
    private JButton getButton(){
        if(button1==null){
            ...
            button1.setText("OK");
            button1.setToolTipText("OK!!!");//设置鼠标在Label上停留时显示提示信息
    ...
    public static void main(String[] args) {
		 Demo demo = new Demo();
		 demo.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//一定要设置关闭
